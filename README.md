...
**Languages**
`Python` `SQL` `SparkSQL` `Cypher`

**Cloud**
`AWS (S3, Glue, Redshift, EMR, Batch, Lambda)` `Azure (ADF, Databricks)` `GCP (BigQuery)`

**Data & Streaming**
`Spark` `PySpark` `Airflow` `Kafka` `Kinesis` `Snowflake` `dbt` `Delta Lake` `Apache Iceberg` `Hadoop` `Hive`

**Databases & Tools**
`PostgreSQL` `Neo4j` `Docker` `Power BI` `Git` `Azure DevOps CI/CD`

**ML & Libraries**
`PyTorch` `TensorFlow` `scikit-learn` `XGBoost` `NLP` `Pandas`
